...
public class GameWorld extends GameEngine {
    ...
    public void initialize() {
        getGameNodes().getChildren().add(this.level.getBackground());
    ...
    private void setupInput() {
        ...
        this.getSceneNodes().setOnMousePressed((MouseEvent event) -> {
            if (!isGameOver()) {
        ...
        this.getSceneNodes().setOnKeyPressed((KeyEvent event) -> {
        ...
        this.getSceneNodes().setOnKeyReleased(event -> {
            vKeys.put(event.getCode(), false);
            this.level.getTank().plotCourse(vKeys, true);

        });

        this.getSceneNodes().setOnMouseMoved(
            (e) -> this.level.getTank().aimAt(e.getSceneX(), e.getSceneY())
        );
    }
    
    @Override
    protected void deinitialize() {
        this.getSceneNodes().setOnMousePressed(null);
        this.getSceneNodes().setOnMousePressed(null);
        this.getSceneNodes().setOnKeyReleased(null);
        this.getSceneNodes().setOnMouseMoved(null);
    }
    ...
    private void generateManySpheres(int numSpheres) {
        ...
        Bounds gameSurface = getGameNodes().getLayoutBounds();
    ...
    protected void handleUpdate(Sprite sprite) {
        if (isGameOver()) {
            ...
            if (sprite == this.level.getTank()) {
                this.getGameNodes().setTranslateX(
                    this.getGameNodes().getScene().getWidth() / 2 -
                    this.level.getTank().getCenterX()
                );
                
                this.getGameNodes().setTranslateY(
                    this.getGameNodes().getScene().getHeight() / 2 -
                    this.level.getTank().getCenterY()
                );
            }
    ...
    private void bounceOffWalls(Sprite sprite) {
        ...
        if (sprite.getNode().getTranslateX() > (this.level.getBackground().getWidth() - displayNode.getBoundsInParent().getWidth())
        ...
        if (sprite.getNode().getTranslateY() > this.level.getBackground().getHeight() - displayNode.getBoundsInParent().getHeight()
    ...
    private void removeMissiles(Missile missile) {
        // bounce off the walls when outside of boundaries
        if (missile.getNode().getTranslateX() > (this.level.getBackground().getWidth()
        ...
        if (missile.getNode().getTranslateY() > this.level.getBackground().getHeight()
